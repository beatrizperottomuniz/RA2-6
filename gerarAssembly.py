...
if __name__ == '__main__':
    ...
    def t(tipo, linha=1, coluna=1, sid=0):
        return Token(tipo, linha, coluna, sid)

    from globalVars import string_pool_global as pool
    ids = {lex: pool.buscarOuAdicionar(lex)
           for lex in ["3", "4", "5", "2", "1", "10", "3.14", "2.0"]}

    def tk_num(lex, linha=2):
        tipo = TokenType.NUM_FLOAT if '.' in lex else TokenType.NUM_INT
        return Token(tipo, linha, 1, ids[lex])

    def montar(corpo_tokens):
        return ([t(TokenType.LPAREN), t(TokenType.KEYWORD_START), t(TokenType.RPAREN)]
                + corpo_tokens
    ...
    testes = {
        "aritmetica": montar([
            t(TokenType.LPAREN, 2), tk_num("3"), tk_num("4"),
            t(TokenType.PLUS, 2), t(TokenType.RPAREN, 2),
        ]),
        "atribuicao": montar([
            t(TokenType.LPAREN, 2), tk_num("5"),
            Token(TokenType.ID, 2, 1, pool.buscarOuAdicionar("X")),
            t(TokenType.RPAREN, 2),
        ]),
        "if": montar([
            t(TokenType.LPAREN, 2),
                t(TokenType.LPAREN, 2), tk_num("5"), tk_num("5"),
                t(TokenType.EQ, 2), t(TokenType.RPAREN, 2),
                t(TokenType.LPAREN, 2), tk_num("1"), tk_num("2"),
                t(TokenType.PLUS, 2), t(TokenType.RPAREN, 2),
                t(TokenType.KEYWORD_IF, 2),
            t(TokenType.RPAREN, 2),
        ]),
        "for": montar([
            t(TokenType.LPAREN, 2),
                ...
                t(TokenType.LPAREN, 2), tk_num("1"), tk_num("2"),
                t(TokenType.PLUS, 2), t(TokenType.RPAREN, 2),
                t(TokenType.KEYWORD_FOR, 2),
            t(TokenType.RPAREN, 2),
        ]),
    }

    for nome, tokens in testes.items():
        resultado = parsear(tokens, tabela)
        if resultado['erros']:
            print(f"[{nome}] Erros no parser:", resultado['erros'])
            continue
        arvore = gerarArvore(resultado['estrutura_derivacao'])
        saida  = f"saida2_{nome}.s"
        gerarAssembly(arvore)
        print(f"[{nome}] OK -> {saida}")
